...
@synthesize operationURLSuccessBlock = _operationURLSuccessBlock;
...
- (void)performCompletionCallback
{
    if (self.error == nil)
    {
        ...
        if (self.operationURLSuccessBlock)
        {
            self.operationURLSuccessBlock(self.fileID, self.dataURL);
        }
    }
    else
    {
...
- (void)finishDataOperationWithURL:(NSURL *)url
{
    self.dataURL = url;
    [self close];
    [self.connection cancel];
...
- (void)connection:(NSURLConnection *)connection didReceiveResponse:(NSURLResponse *)response
{
    ...
    if(self.operationURLSuccessBlock && self.error==nil){
        [self finishDataOperationWithURL:[response URL]];
        return;
    }
