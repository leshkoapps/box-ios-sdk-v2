...
#ifndef BOX_DEFAULT_TIME_SEPARATOR
#	define BOX_DEFAULT_TIME_SEPARATOR ':'
#endif
unichar BoxISO8601DefaultTimeSeparatorCharacter = BOX_DEFAULT_TIME_SEPARATOR;

//Unicode date formats.
#define BOX_ISO_CALENDAR_DATE_FORMAT @"yyyy-MM-dd"
//#define ISO_WEEK_DATE_FORMAT @"YYYY-'W'ww-ee" //Doesn't actually work because NSDateComponents counts the weekday starting at 1.
#define BOX_ISO_ORDINAL_DATE_FORMAT @"yyyy-DDD"
#define BOX_ISO_TIME_FORMAT @"HH:mm:ss"
#define BOX_ISO_TIME_WITH_TIMEZONE_FORMAT  BOX_ISO_TIME_FORMAT @"Z"
//printf formats.
#define BOX_ISO_TIMEZONE_UTC_FORMAT @"Z"
#define BOX_ISO_TIMEZONE_OFFSET_FORMAT @"%+.2d%.2d"

@interface BoxISO8601DateFormatter(UnparsingPrivate)
...
@implementation BoxISO8601DateFormatter
...
- (NSDateComponents *) dateComponentsFromString:(NSString *)string timeZone:(out NSTimeZone **)outTimeZone range:(out NSRange *)outRange {
	...
	NSDateComponents *nowComponents = [parsingCalendar components:(NSCalendarUnitYear | NSMonthCalendarUnit | NSCalendarUnitDay) fromDate:now];
	...
	if (strict) timeSep = BoxISO8601DefaultTimeSeparatorCharacter;
    ...
    // bail out if the date is invalid
    if (!isValidDate)
    {
        return nil;
    }
...
- (NSString *) stringFromDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	switch (self.format) {
		case BoxISO8601DateFormatCalendar:
			return [self stringFromDate:date formatString:BOX_ISO_CALENDAR_DATE_FORMAT timeZone:timeZone];
		case BoxISO8601DateFormatWeek:
			return [self weekDateStringForDate:date timeZone:timeZone];
		case BoxISO8601DateFormatOrdinal:
			return [self stringFromDate:date formatString:BOX_ISO_ORDINAL_DATE_FORMAT timeZone:timeZone];
		default:
			[NSException raise:NSInternalInconsistencyException format:@"self.format was %lu, not calendar (%d), week (%d), or ordinal (%d)", (unsigned long)self.format, BoxISO8601DateFormatCalendar, BoxISO8601DateFormatWeek, BoxISO8601DateFormatOrdinal];
			return nil;
	}
}
...
- (NSString *) stringFromDate:(NSDate *)date formatString:(NSString *)dateFormat timeZone:(NSTimeZone *)timeZone {
	if (includeTime)
		dateFormat = [dateFormat stringByAppendingFormat:@"'T'%@", [self replaceColonsInString:BOX_ISO_TIME_FORMAT withTimeSeparator:self.timeSeparator]];
	...
	if (includeTime) {
		...
		if (offset == 0)
			str = [str stringByAppendingString:BOX_ISO_TIMEZONE_UTC_FORMAT];
		else
			str = [str stringByAppendingFormat:BOX_ISO_TIMEZONE_OFFSET_FORMAT, (int)(offset / 60), (int)(offset % 60)];
	}
...
- (NSString *) weekDateStringForDate:(NSDate *)date timeZone:(NSTimeZone *)timeZone {
	...
	if(includeTime) {
		...
		unichar timeSep = self.timeSeparator;
		if (!timeSep) timeSep = BoxISO8601DefaultTimeSeparatorCharacter;
		formatter.dateFormat = [self replaceColonsInString:BOX_ISO_TIME_WITH_TIMEZONE_FORMAT withTimeSeparator:timeSep];
